...
startpgmcanny3(char *infilename, float sigma, float tlow, float thigh, char *dirfilename)
{
...
//			        suppression. */
   ...
   ****************************************************************************/
   ...
   canny3(image, rows, cols, sigma, tlow, thigh, &edge, dirfilename);
...
///////////////
startjpgcanny3(char *infilename, float sigma, float tlow, float thigh, char *dirfilename)
{
//   char *infilename = NULL;  /* Name of the input image */
//   char *dirfilename = NULL; /* Name of the output gradient direction image */
   char outfilename[128];    /* Name of the output "edge" image */
...
//			        suppression. */
   ...
   ****************************************************************************/
   if(VERBOSE) printf("Reading the image %s.\n", infilename);
   if(read_jpg_image(infilename, &image, &rows, &cols) == 0){
      fprintf(stderr, "Error reading the input image, %s.\n", infilename);
      exit(1);
   }
...
//////////////////////////////////////
int read_jpg_image(char *infilename, unsigned char **image, int *rows,
    int *cols)
{
    Image *image1;
	Image *image2;
	...
	int H, W;
	int i,j;
	InitializeMagick(infilename);
	GetExceptionInfo(&exception);
	image_info=CloneImageInfo((ImageInfo *) NULL);
	strcpy(image_info->filename, infilename);

	image1=ReadImage(image_info,&exception);

  	H = image1->rows;  W = image1->columns;
	*rows = H;  *cols = W;

	pixels = GetAuthenticPixels(image1,0,0,W,H,&exception);

    if(((*image) = (unsigned char *) malloc((H)*(W))) == NULL){
		exit(0);
	}
	for(i=0; i<H; i++ )
	{
		for(j=0; j<W; j++)
		{
			(*image)[i*W+j] = (unsigned char)(pixels[i*W+j].blue / 256);
		}
	}
}
//////////////////////////////
int write_jpg_image(char *outfilename, unsigned char *image, int rows,
    int cols, char *comment, int maxval)
{
    Image *image1;
	Image *image2;
	...
	InitializeMagick(outfilename);
	GetExceptionInfo(&exception);

	image_info2=CloneImageInfo((ImageInfo *) NULL);
	strcpy(image_info2->filename, outfilename);\
	GetExceptionInfo(&exception);
	image_info2->colorspace = GRAYColorspace;
	GetExceptionInfo(&exception);
	image2 = ConstituteImage(cols, rows,"I", CharPixel, image, &exception);
	GetExceptionInfo(&exception);
