...
void    remove_pixels_along_line(double l_theta, double l_rho, IMAGE *aImage, int theta_res, int rho_res, int **histogram, int color_index, Window theWindow, GC theGC);
BOOLEAN find_line_ends(double theta, double rho, IMAGE *aImage, int *x_start, int *y_start, int *x_end, int *y_end);
double  find_density_of_black_pixels(int v1, int width, double m, double b, char flag, IMAGE *aImage);



void
find_lines_with_SHT(IMAGE *aImage, XImage *aXImage, Window theWindow, GC theGC)
{
    int     num_lines;
    int     t, r;
    ...
    SHT_hough_transform(aImage, theta_res, rho_res, histogram);
    ...
    while (h_max > background)
        ...
        remove_pixels_along_line(theta, rho, aImage, theta_res, rho_res, histogram, color_index, theWindow, theGC);
    ...
    if (error == TRUE)
        ...
        display_message_and_wait(theWindow, 0, 0, 10,
                            "",
                            "Standard HT algorithm for line detection",
        ...
        {
        if (num_lines == 1)
            sprintf(results_message, "Found 1 line with SHT.");
        else
            sprintf(results_message, "Found %d lines with SHT.", num_lines);
...
void
free_histogram(int **histogram, int x_max)
{
    int x;
 ...
 */
void
SHT_hough_transform(IMAGE *aImage, int theta_res, int rho_res, int **histogram)
{
    int x, y;
    double  theta, rho, max_rho;
    int t, r;
 ...
 */
void
remove_pixels_along_line(double l_theta, double l_rho, IMAGE *aImage, int theta_res, int rho_res, int **histogram, int color_index, Window theWindow, GC theGC)
{
    int     x, y;
    double  m, b;
    double  theta_res_d, rho_res_d; /* (double ) form of theta_res, rho_res */
    ...
    (void )find_line_ends(l_theta, l_rho, aImage,
    ...
    if (ABS(m) <= 1.0)
        ...
        for (x=x_start; x < x_end; x++)
            ...
            for (j=y-width/2; j <= y+width/2; j++)
                ...
                if ((j >= 0) && (j < aImage->y))
                    {
                    /* If BLACK and untagged
                     */
                    if ((get_pixel_value(aImage, x, j) == BLACK) &&
                            (test_pixel_for_tag(aImage, x, j, 1) == FALSE))
                        {
            ...
            for (i=x-width/2; i <= x+width/2; i++)
                ...
                if ((i >= 0) && (i < aImage->x))
                    ...
                    if ((get_pixel_value(aImage, i, y) == BLACK) &&
                            (test_pixel_for_tag(aImage, i, y, 1) == FALSE))
                        {
                        /* Remove its contribution from the histogram.
                         */
                        for (t=0; t < theta_res; t++)
...
}       /* end of remove_pixels_along_line() */



BOOLEAN
find_line_ends(double theta, double rho, IMAGE *aImage, int *x_start, int *y_start, int *x_end, int *y_end)
{
    int     x, y, sz;
    double  m, b;
    int     width = 15;
    ...
    if (ABS(m) <= 1.0)
        ...
        do {
            ...
            for (; x < aImage->x; x++)
                {
                density = find_density_of_black_pixels(x, width, m, b, 'x', aImage);
            ...
            for (; x < aImage->x; x++)
                ...
                if (density < threshold)
                    ...
                    for (sz=0;(((x+sz) < aImage->x) && (density < threshold)); sz++)
                        density = find_density_of_black_pixels((x+sz), width, m, b, 'x', aImage);
        ...
        do {
            ...
            for (; y < aImage->y; y++)
                {
                density = find_density_of_black_pixels(y, width, m, b, 'y', aImage);
                if (density > threshold)
                    break;
                }
            *y_start = y;
            ...
            for (; y < aImage->y; y++)
                ...
                if (density < threshold)
                    ...
                    for (sz=0;(((y+sz) < aImage->y) && (density < threshold)); sz++)
                        density = find_density_of_black_pixels((y+sz), width, m, b, 'y', aImage);
...
}       /* end of find_line_ends() */



double
find_density_of_black_pixels(int v1, int width, double m, double b, char flag, IMAGE *aImage)
{
    int count = 0;
    int x, y, i, j;
    double  density;
...
}   /* end of find_density_of_black_pixels() */
